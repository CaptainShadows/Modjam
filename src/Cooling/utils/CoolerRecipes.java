...
public class CoolerRecipes {
    ...
    private CoolerRecipes() {
        ...
        this.addCooling(Item.rottenFlesh.itemID,
                new ItemStack(Item.ingotIron), 0.7F);
     ...
     */
    public void addCooling(int par1,
            ItemStack par2ItemStack, float par3) {
        this.CoolingList.put(Integer.valueOf(par1),
                par2ItemStack);
        this.experienceList.put(
                Integer.valueOf(par2ItemStack.itemID),
                Float.valueOf(par3));
    }
     ...
     */
    public void addCooling(int itemID, int metadata,
            ItemStack itemstack, float experience) {
        metaCoolingList.put(
                Arrays.asList(itemID, metadata), itemstack);
        metaExperience.put(Arrays.asList(itemID, metadata),
                experience);
    }
    ...
    public ItemStack getCoolingResult(ItemStack item) {
        ...
        ItemStack ret = (ItemStack) metaCoolingList
                .get(Arrays.asList(item.itemID,
                        item.getItemDamage()));
        if (ret != null){
            return ret;
        }
        return (ItemStack) CoolingList.get(Integer
                .valueOf(item.itemID));
    }
    ...
    public float getExperience(ItemStack item) {
        ...
        float ret = item.getItem().getSmeltingExperience(
                item);
        if (ret < 0
                && metaExperience.containsKey(Arrays
                        .asList(item.itemID,
                                item.getItemDamage()))){
            ret = metaExperience.get(Arrays.asList(
                    item.itemID, item.getItemDamage()));
        }
        if (ret < 0
                && experienceList.containsKey(item.itemID)){
